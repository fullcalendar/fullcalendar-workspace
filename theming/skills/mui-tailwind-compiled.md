...
converting
  INPUT: /theming/ui-mui-tailwind/src/theme-breezy/EventCalendarView.tsx
  OUTPUT: /theming/ui-mui-tailwind/src/theme-breezy/_compiled/EventCalendarView.tsx
...
NOTE: you may be instructed to do this for a different theme other then breezy. If so, please replace all references to "breezy" in paths with the provided theme name. Here is the general form
  INPUT: /theming/ui-mui-tailwind/src/theme-<themename>/EventCalendarView.tsx
  OUTPUT: /theming/ui-mui-tailwind/src/theme-<themename>/_compiled/EventCalendarView.tsx
...
DO inline all symbols from:
  ...
  Symbols from the /theming/ui-mui-tailwind/src/lib directory
...
Exporting consts for Scheduler reuse
  When initially creating the EventCalendarView.tsx compiled file, do NOT export any consts or functions (except EventCalendarProps and the main component function). Wait until the SchedulerView.tsx conversion is requested. Only then, determine which consts/functions need to be exported from EventCalendarView.tsx for reuse in SchedulerView.tsx, and add those exports.


## Part 2: Converting the SchedulerView.tsx file

Do this only if explicitly asked.
Do the same instructions I wrote for converting EventCalendarView.tsx, but for SchedulerView.tsx instead.

converting
  INPUT: /theming/ui-mui-tailwind/src/theme-<themename>/SchedulerView.tsx
  OUTPUT: /theming/ui-mui-tailwind/src/theme-<themename>/_compiled/SchedulerView.tsx

Instead of inline all consts afresh, try to export and reuse what's already in _compiled/EventCalendarView.tsx
When merging props, ensure resourceExpanderContent goes right after resourceExpanderClass


## Part 3: Converting ancillary files

Do this only if explicitly asked.

Move /theming/ui-mui-tailwind/src/lib/EventCalendarToolbar.tsx into _compiled as-is.

Then, copy /theming/ui-mui-tailwind/src/theme-<themename>/EventCalendar.tsx into _compiled
  Ensure you inline any symbols from @fullcalendar/theme-common,
  but do NOT recursively inline. Just keep the top-level consts, functions, and types.
  Ensure you reference the new EventCalendarToolbar.tsx in _compiled
  Ensure you reference the new EventCalendarView.tsx in _compiled
  Ensure you inline the EventCalendarProps interface

The, copy /theming/ui-mui-tailwind/src/theme-<themename>/Scheduler.tsx into _compiled
  Ensure you inline any symbols from @fullcalendar/theme-common,
  but do NOT recursively inline. Just keep the top-level consts, functions, and types.
  Ensure you reference the new EventCalendarToolbar.tsx in _compiled
  Ensure you reference the new SchedulerCalendarView.tsx in _compiled
  Ensure you inline the SchedulerProps interface

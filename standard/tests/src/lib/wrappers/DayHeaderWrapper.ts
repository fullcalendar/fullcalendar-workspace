...
export class DayHeaderWrapper {
  ...
  getCellEl(dateOrDow) {
    if (typeof dateOrDow === 'number') {
      return this.el.querySelector(`.fcnew-cell.${CalendarWrapper.DOW_CLASSNAMES[dateOrDow]}`)
    }
    if (typeof dateOrDow === 'string') {
      dateOrDow = parseUtcDate(dateOrDow)
    }
    return this.el.querySelector(`.fcnew-cell[data-date="${formatIsoDay(dateOrDow)}"]`)
  }
  ...
  getCellInfo() { // all
    return this.getCellEls().map((cellEl) => ({
      ...
      isToday: cellEl.classList.contains('fcnew-day-today'),
    }))
  }

  getNavLinkEls() {
    return findElements(this.el, '.fcnew-cell[data-date] a[data-navlink]')
  }
  ...
  getNavLinkEl(dayDate) {
    ...
    return this.el.querySelector('.fcnew-cell[data-date="' + formatIsoDay(dayDate) + '"] a')
  }

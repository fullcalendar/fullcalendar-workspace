			...
			initCalendar
				eventAfterAllRender: -> setTimeout ->
					$.simulateByPoint 'drag',
						point: getTimeGridPoint('mon', 0, '2am')
						endPoint: getTimeGridPoint('mon', 0, '4am')
						callback: ->
	...
	describe 'with resource columns above date columns', ->
		...
		it 'allows a same-day resource selection', (done) ->
			...
			initCalendar
				eventAfterAllRender: ->
					$.simulateByPoint 'drag',
						point: getTimeGridPoint('sun', 1, '2am')
						endPoint: getTimeGridPoint('sun', 1, '4am')
						callback: ->
							expect(selectCalled).toBe(true)
							done()
				select: (start, end, jsEvent, view, resource) ->
		...
		it 'allows a different-day resource selection', (done) ->
			...
			initCalendar
				eventAfterAllRender: ->
					$.simulateByPoint 'drag',
						point: getTimeGridPoint('sun', 1, '2am')
						endPoint: getTimeGridPoint('mon', 1, '4am')
						callback: ->
							expect(selectCalled).toBe(true)
							done()
				select: (start, end, jsEvent, view, resource) ->
	...
	describe 'with date columns above resource columns', ->
		...
		it 'allows a same-day resource selection', (done) ->
			...
			initCalendar
				eventAfterAllRender: ->
					$.simulateByPoint 'drag',
						point: getTimeGridPoint('mon', 1, '2am')
						endPoint: getTimeGridPoint('mon', 1, '4am')
						callback: ->
							expect(selectCalled).toBe(true)
							done()
		...
		it 'disallows a selection across resources', (done) ->
			...
			initCalendar
				eventAfterAllRender: ->
					$.simulateByPoint 'drag',
						point: getTimeGridPoint('sun', 0, '2am')
						endPoint: getTimeGridPoint('sun', 1, '4am')
						callback: ->
							expect(selectCalled).toBe(false)
							done()

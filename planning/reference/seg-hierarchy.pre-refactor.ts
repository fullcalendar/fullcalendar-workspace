/*
FROZEN REFERENCE — do not edit, do not import.
Extracted from commit ec36941a1~1 (the state immediately before the
"Phase 1: shared geometry kernel" refactor commit):

  git show 'ec36941a1~1:standard/packages/preact/src/seg-hierarchy.ts'

This is the fire/collide/peel/consume insertion model (with zombies) that the
new merge engine revives, minus zombies, whose slot-blocking role is replaced
by more-link occupants. See planning/INDEX.md and pseudocode.js.
All other pre-refactor code is reachable the same way, e.g.:

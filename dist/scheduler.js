...
/*!
FullCalendar Scheduler v1.1.0-beta
...
ResourceGrid = (function(superClass) {
  ...
  ResourceGrid.prototype.computeSelection = function(startSpan, endSpan) {
    var selectionRange;
    if (!this.allowCrossResource && startSpan.resourceId !== endSpan.resourceId) {
      return;
    }
...
ResourceDayTableMixin = {
  ...
  flattenResources: function(resources) {
    var orderSpecs, res, sortFunc;
    orderSpecs = parseFieldSpecs(this.view.opt('resourceOrder'));
  ...
  renderHeadTrHtml: function() {
    if (!this.resourceCnt) {
      ...
      if (this.daysPerRow > 1) {
        if (this.datesAboveResources) {
          return this.renderHeadDateTrHtml(1, this.resourceCnt) + this.renderHeadResourceTrHtml(this.daysPerRow);
        } else {
  ...
  renderHeadResourceCellHtml: function(resource, colspan) {
    ...
    return '<th class="fc-resource-cell"' + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + '>' + htmlEscape(this.view.getResourceText(resource)) + '</th>';
  },
...
ResourceDayGrid = (function(superClass) {
  ...
  ResourceDayGrid.prototype.rangeToSegs = function(range) {
    var copy, genericSegs, j, k, len, ref, resourceCnt, resourceIndex, resourceObj, resourceSegs, seg;
    genericSegs = this.datesAboveResources ? this.sliceRangeByDay(range) : this.sliceRangeByRow(range);
    ...
    if (!resourceCnt) {
      ...
      for (j = 0, len = genericSegs.length; j < len; j++) {
        ...
        for (resourceIndex = k = 0, ref = resourceCnt; k < ref; resourceIndex = k += 1) {
          ...
          if (!range.resourceId || range.resourceId === resourceObj.id) {
            copy = $.extend({}, seg);
            if (this.isRTL) {
...
ResourceTimeGrid = (function(superClass) {
  ...
  ResourceTimeGrid.prototype.rangeToSegs = function(range) {
    var copy, genericSegs, j, k, len, ref, resourceCnt, resourceIndex, resourceObj, resourceSegs, seg;
    genericSegs = this.sliceRangeByTimes(range);
    resourceCnt = this.resourceCnt;
    if (!resourceCnt) {
      return genericSegs;
    } else {
...
TimelineGrid = (function(superClass) {
  ...
  TimelineGrid.prototype.buildCoords = function() {
    ...
    return this.slatCoordCache.build();
  };

  TimelineGrid.prototype.computeDateSnapCoverage = function(date) {
    var snapCoverage, snapDiff, snapDiffInt, snapDiffRemainder;
  ...
  TimelineGrid.prototype.dateToCoord = function(date) {
    var partial, slotCoverage, slotIndex, snapCoverage;
    date = date.clone().stripZone();
    ...
    if (this.isRTL) {
      return (this.slatCoordCache.getRightPosition(slotIndex) - this.slatCoordCache.getWidth(slotIndex) * partial) - this.containerCoordCache.getWidth(0);
    } else {
      return this.slatCoordCache.getLeftPosition(slotIndex) + this.slatCoordCache.getWidth(slotIndex) * partial;
    }
  };
